"""Benchmark WebSocket TTS TTFB across providers using LiveKit stream() API.

Simulates LLM token-by-token output. The TTS service's internal sentence
tokenizer aggregates tokens into sentences before sending to the provider.
Uses 2 sentences to validate the tokenizer flushes the first sentence
immediately without waiting for the second.
"""
...
async def benchmark_stream(
    ...
    try:
        ...
        try:
            await asyncio.wait_for(_consume_audio(), timeout=30.0)
        except asyncio.TimeoutError:
            print(f"\n⚠️  {service_name}: stream timed out after 30s")
        ...
        if save_audio and all_audio:
            ...
            save_wav(bytes(all_audio), f"{safe_name}_ws.wav", sample_rate, output_dir)
...
async def main():
    ...
    async def _bench_service(sid, cfg, api_key):
        ...
        for iteration in range(total_iters):
            ...
            try:
                result = await benchmark_stream(
                    tts, text, cfg["name"], args.token_delay,
    ...
    print_results(aggregated, "WEBSOCKET TTS BENCHMARK RESULTS (LiveKit Python)")
