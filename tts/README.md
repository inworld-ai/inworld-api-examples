...
| **WebSocket streaming** | Persistent connection; audio chunks stream over the same connection. | **Lowest latency**; best for real-time voice apps, agents, and interactive use. | `example_tts_low_latency_ws`, `example_websocket` |

In general: **WebSocket** has the lowest time-to-first-byte (TTFB), **HTTP streaming** is next, and **non-streaming HTTP** is highest latency but simplest. For more on latency and best practices, see [Generating speech – Latency](https://docs.inworld.ai/docs/tts/best-practices/generating-speech#latency).
