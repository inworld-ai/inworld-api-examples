 ...
 * WebSocket API. For WAV input use example_stt_websocket.js instead.
 */
 ...
 */
function streamTranscribePcm(pcmPath, sampleRate, channels, apiKey, options = {}) {
...
async function main() {
    console.log('Inworld STT WebSocket Transcription Example (raw PCM)');
    ...
    if (!fs.existsSync(pcmPath)) {
        ...
        console.log('Usage: node example_stt_websocket_pcm.js [pcm.raw] [sampleRate] [channels]');
    ...
    try {
        ...
        const { finalTexts } = await streamTranscribePcm(pcmPath, sampleRate, channels, apiKey);
